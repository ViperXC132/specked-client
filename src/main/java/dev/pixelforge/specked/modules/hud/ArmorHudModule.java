...
public class ArmorHudModule extends Module {
    ...
    public void onHudRender(DrawContext ctx, float tickDelta) {
        ...
        int baseX = ((Number) settings.get(0).getValue()).intValue();
        int baseY = ((Number) settings.get(1).getValue()).intValue();
        ...
        for (EquipmentSlot slot : SLOTS) {
            ...
            ctx.drawStackOverlay(mc.textRenderer, stack, baseX, y);
            if (isEnchanted(stack)) ctx.fill(baseX + 14, y, baseX + 16, y + 2, 0xFF55FFFF);
